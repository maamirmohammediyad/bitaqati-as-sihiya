...
function CompleteAccountRecoveryContent() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(
        `${API_URL}/auth/account-recovery/complete`,
        {
          method: 'POST',
          headers: {
          ...
          <div className="p-7 sm:p-8">
            {isSuccess ? (
              ...
              <>
                ...
                <p className="mt-3 text-sm leading-7 text-slate-600">
                  اختر كلمة مرور قوية لا تقل عن 8 أحرف، وتحتوي على حروف
                  وأرقام.
                </p>

                <p className="mt-2 text-xs leading-6 text-slate-500">
                  البريد الإلكتروني مطلوب فقط إذا لم يكن مسجلاً سابقًا في
                  حسابك.
                </p>
  ...
  return (
    <Suspense
      fallback={
        ...
        >
          <div className="rounded-xl bg-white px-6 py-5 text-sm text-slate-600 shadow">
            جارٍ تحميل الصفحة...
          </div>
        </main>
      }
    >
      <CompleteAccountRecoveryContent />
    </Suspense>
  );
}